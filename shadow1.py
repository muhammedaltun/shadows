...
def main():
    ...
    drawGrid()
    for p in points:
        pygame.draw.circle(DISPLAYSURF, YELLOW, p, 8, 0)
        ...
        for event in pygame.event.get():       # event handling loop
            if event.type == QUIT:
                ...
                if event.key == K_RETURN: 
                    DISPLAYSURF.fill(GRAY)
                    for cell in painted: pygame.draw.polygon(DISPLAYSURF,DARK,cellShadow(points[0],cell))
                    for part in allParts(painted):
                        
                        back = boundary(part)
                        for i in range(len(back)-1):
                            pygame.draw.line(DISPLAYSURF,RED,back[i],back[i+1],6)
                        colour = (random.randint(0,255),random.randint(0,255),random.randint(0,255))                       
                        pygame.draw.polygon(DISPLAYSURF,colour,back)
                    drawGrid() 
                    for p in points:
                        pygame.draw.circle(DISPLAYSURF, YELLOW, p, 8, 0)
...
def boundary(obs):
    # obs: a connected set of cells
    first = list(importantVertices((0,0),obs))[0]
    lis = [first]
    for n in neighbors(first):
        if len(set(edgeNeighbors(first,n)).intersection(obs)) == 1 :
                second = n
                break
    lis.append(second) 
    if edgeNeighbors(first,second)[0] in obs: side = 0
    else: side = 1
    otherSide = abs(side-1)

    while lis[-1] != first:          
        news = []                                        
        for n in neighbors(lis[-1]):
            if n != lis[-2] and edgeNeighbors(lis[-1],n)[side] in obs and not edgeNeighbors(lis[-1],n)[otherSide] in obs: news.append(n)
        if len(news) == 1: lis.append(news[0])
        
        elif len(news) == 2:
            if edgeNeighbors(lis[-1],news[0])[side] == edgeNeighbors(lis[-2],lis[-1])[side]: lis.append(news[1])
            else: lis.append(news[0])
